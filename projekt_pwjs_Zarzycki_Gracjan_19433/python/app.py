...
@login_required
def main():
    dane = {'tytul': 'Strona główna', 'tresc': 'Strona główna 19433'}
...
@login_required
def dodaj():
    dane = {'tytul': 'Dodaj pracownika', 'tresc': 'Podaj dane pracownika:'}
...
def addrec():
    if request.method == 'POST':
        try:
            ...
            with sql.connect("sqllite.db") as con:
                cur = con.cursor()
...
@login_required
def lista():
    dane = {'tytul': 'Lista pracownikow', 'tresc': 'Dane pracownikow:'}
    con = sql.connect('sqllite.db')
